...
void setup()
{
    ...
    // ESC (arms during initESC — takes 3 s)
    initESC();
...
void loop()
{
    ...
    if (digitalRead(PIN_RESET_BTN) == LOW)
    {
        ...
        if (digitalRead(PIN_RESET_BTN) == LOW)
        {
            ...
            resetReference();
            while (digitalRead(PIN_RESET_BTN) == LOW) {}  // wait for release
        }
    }
    ...
    if (readMPU6050())
    {
        ...
        // Seed reference on first valid IMU reading
    ...
    {
        // tiltMag = largest single-axis deviation (magnitude only)
